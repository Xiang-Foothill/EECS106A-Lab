...
class TurtleBotController(Node):
    ...
    def loop(self):
        try:
            tf = self.tf_buffer.lookup_transform(self.ar_frame, self.turtle_frame, Time())
            delta_x = tf.transform.translation.x
            delta_y = tf.transform.translation.y

            x_vel = self.K1*delta_x
            theta_vel = self.K2*delta_y

            control_cmd = Twist()
            control_cmd.linear.x = x_vel
            control_cmd.angular.z = theta_vel
